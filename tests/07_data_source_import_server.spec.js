...
// 2. FILTER records for Script 07
const records = allRecords.filter(row => row.SCRIPT_NO === '07');

// 3. Loop through each row and register tests dynamically at the top level
for (const row of records) {
    test(`TC-${row.SCRIPT_NO}: API-Driven Server creation & Dynamic Instance Verification (${row.IMPORT_JOB_RDB_DATASOURCE})`, async ({ page }) => {
        test.setTimeout(300_000);

        const snUrl = process.env.SN_URL;
        const testAssetName = row.IMPORT_JOB_RDB_DATASOURCE;   
        const testAssetServer = row.IMPORT_JOB_RDB_SERVER;    
        const rdbType = row.IMPORT_JOB_RDB_TYPE;               
        const rdbUrl = row.RDB_URL;                        
...
// test('API-Driven Server creation & Dynamic Instance Verification', async ({ page }) => {
//     test.setTimeout(300_000);
