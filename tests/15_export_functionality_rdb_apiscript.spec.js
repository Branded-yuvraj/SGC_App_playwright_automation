...
test(`Server and Instance export workflow via API (${row.EXPORT_SERVER_INSTANCE})`, async ({ page }) => {
    ...
    // Update first configuration textbox (nth 3): serverName if PDB, instanceName otherwise
    ...
    const valueNth3 = isOraclePdb ? serverName : instanceName;
    await configTextBox1.fill(valueNth3);
    console.log(`[Config] Filled nth(3) with: ${valueNth3}`);

    // Update second configuration textbox (nth 5): instanceName if PDB, cleared otherwise
    ...
    if (isOraclePdb) {
        ...
        console.log(`[Config] Filled PDB instance name into nth(5): ${instanceName}`);
    } else {
        await configTextBox2.fill('');
        console.log(`[Config] Cleared nth(5) since record is not Oracle PDB.`);
    }
    ...
    // --- 6. Navigate to Logs, Filter by Message, and Assert Final Message ---
    await page.getByRole('menuitem', { name: 'All' }).click();
    if (await clearFilterButton.isVisible().catch(() => false)) {
        await clearFilterButton.click();
    }
    ...
    const searchContext = gsftMain.locator('input.form-control.list-search').first();
    if (await searchContext.isVisible({ timeout: 5_000 }).catch(() => false)) {
        await searchContext.fill('Service Graph Connector');
        await searchContext.press('Enter');
        await page.waitForTimeout(3_000);
    }
